...
async function initOfflineVoice() {
    try {
        ...
        let _lastForwardedAt = 0;        // Timestamp of last text chunk forwarded to Gemini
        const FORWARD_COOLDOWN_MS = 4000; // Block new forwards for 4s after each forward
        ...
        recognizer.on("result", (message) => {
            ...
            if (window.novaState.isAwake && text.match(/\b(pause|stop|resume|volume|lower|higher|quieter|louder|increase|decrease|scroll|close|open)\b/i)) {
                ...
                // When Live session has ended but isAwake is still true (session dropped after reply),
                // "close the browser" must be handled directly — processCommand hits WAKE OVERRIDE.
                if (/\bclose\b.*\bbrowser\b|\bbrowser\b.*\bclose\b/i.test(text)) {
                    console.log('⚡ Direct browser-close (session idle):', text);
            ...
            // Forward Vosk transcription to Gemini when user is actively awake.
            // 4-second cooldown after each forward prevents background noise from
            // immediately confusing Gemini during multi-step flows like email.
        ...
        function resetLivePlayback() {
            ...
            _lastForwardedAt = 0;          // Gemini finished talking — accept user reply immediately
            listeningSymbol.style.display = 'none';
