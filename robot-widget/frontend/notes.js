...
async function generateNoteContent(userRequest, noteTitle) {
    ...
    console.log('[Notes] Calling backend for note content generation...');
    const text = await backendClient.complete(prompt);
    if (!text || text.trim().length === 0) throw new Error('AI returned empty content');
