// Load environment variables (.env file) before anything else.
// Set NOVA_BACKEND_URL to point to your deployed backend (Heroku URL).
...
// Each user's token is stored locally in their Electron userData directory.
// It is NEVER sent to GitHub or Heroku — only stored on the user's machine.
const TOKEN_FILE = path.join(app.getPath ? app.getPath('userData') : __dirname, 'nova_google_token.json');

function loadLocalToken() {
    try {
        if (fs.existsSync(TOKEN_FILE)) {
            const t = JSON.parse(fs.readFileSync(TOKEN_FILE, 'utf8'));
            backendClient.setGoogleToken(t);
            console.log('[Auth] Loaded local Google token.');
            return t;
        }
    } catch (e) { console.warn('[Auth] Could not load token:', e.message); }
    return null;
}
...
function saveLocalToken(token) {
    try {
        ...
        backendClient.setGoogleToken(token);
        console.log('[Auth] Google token saved locally.');
    } catch (e) { console.warn('[Auth] Could not save token:', e.message); }
}
...
const Automation = {
    ...
    // ── Gmail / Calendar: delegated to backend via live.js ───────────────
    // (These are called from live.js tool handlers via backendClient directly.
    //  The Automation references kept below are for the contacts/calendar UI panels.)
...
function openAuthPanel() {
    ...
    authWin = new BrowserWindow({
        ...
        title: 'Nova — Connect Google Account',
        autoHideMenuBar: true,
        resizable: false,
        webPreferences: { nodeIntegration: true, contextIsolation: false },
    });
    authWin.loadFile(path.join(__dirname, 'google_auth_panel.html'));
    authWin.on('closed', () => { authWin = null; });
}

// Multi-user OAuth flow:
// 1. Get the consent URL from backend (client_id/secret stay on Heroku)
// 2. Start a local HTTP server on port 3141 to catch Google's redirect
// 3. User signs in → Google redirects to localhost:3141 with the code
// 4. Send the code to backend /api/auth/exchange → backend returns token
// 5. Save token locally; inject into all future backend requests
ipcMain.handle('google-auth-start', async () => {
    try {
        if (backendClient.getGoogleToken()) return { already: true };
        const { url } = await backendClient.get('/api/auth/url');
        if (!url) return { error: 'Could not get auth URL from backend.' };

        // Start local callback server on port 3141
            ...
            const srv = http.createServer((req, res) => {
                try {
                    ...
                    res.end(`<html><body style="font-family:sans-serif;background:#0d1117;color:#fff;text-align:center;padding:60px">
                        <h2 style="color:#00ffcc">${c ? '✅ Nova Authorization Complete' : '❌ Authorization failed'}</h2>
                        <p style="color:#8b949e">You can close this window.</p></body></html>`);
                    srv.close();
                    if (c) resolve(c); else reject(new Error(e || 'OAuth denied'));
                } catch (err) { srv.close(); reject(err); }
            });
            srv.listen(3141, 'localhost', () => shell.openExternal(url));
            srv.on('error', reject);
            setTimeout(() => { srv.close(); reject(new Error('OAuth timed out')); }, 5 * 60 * 1000);
        });

        // Exchange code for token via backend (keeps client_secret off the frontend)
        const { tokens } = await backendClient.post('/api/auth/exchange', { code });
        saveLocalToken(tokens);
        return { ok: true };
    } catch (e) {
        console.error('[google-auth-start]', e.message);
        return { error: e.message };
    }
});

ipcMain.on('google-auth-open-panel', () => openAuthPanel());
ipcMain.on('google-auth-close-panel', () => { if (authWin) authWin.close(); });

ipcMain.handle('ask-grok', async (event, text) => {
    try { return await backendClient.chat(text); }
    catch (e) { console.error('❌ ask-grok error:', e.message); return 'Backend unavailable.'; }
});

ipcMain.handle('transcribe-audio', async (event, buffer) => {
    try {
        const base64 = Buffer.isBuffer(buffer) ? buffer.toString('base64') : buffer;
        return await backendClient.transcribeAudio(base64);
    } catch (e) {
        console.error('❌ Transcription error in main:', e.message);
        return '';
    }
});
...
ipcMain.on('dom-map-results', (event, { map, url }) => {
    ...
    // Notify live.js WebSocket client via its callback
    onDomMapAvailable({ map, url });
});
...
ipcMain.handle('generate-speech', async (event, text) => {
    try {
        ...
        console.error('TTS Handle Error:', e.message);
        return null;
    }
});
...
async function generateResearchPaper(topic, mainWindow) {
    ...
    const _ai = { complete: (prompt) => backendClient.complete(prompt) };
    ...
    try {
        ...
        for (let idx = 0; idx < searchQueries.length; idx++) {
            ...
            try {
                const resText = await _ai.complete(searchQueries[idx]);
                gatheredContent.push(resText || '');
            } catch (searchErr) {
...
WRITE THE COMPLETE PAPER NOW:`;
        ...
        for (let attempt = 0; attempt < writeModels.length; attempt++) {
            ...
            try {
                ...
                paperText = await _ai.complete(paperPrompt);
                writeError = null;
                break; // success
            } catch (err) {
                writeError = err;
...
// ── Google Auth IPC (per-user local token) ────────────────────────────────
// Auth status is now based on the local token, not the backend global state.

ipcMain.handle('google-auth-status', () => {
    return { authenticated: !!backendClient.getGoogleToken() };
});
...
ipcMain.handle('calendar-create-event', async (event, { title, startTime, endTime, attendees }) => {
    try {
        return await backendClient.post('/api/calendar', { action: 'create_event', event_title: title, time_expression: startTime, attendees });
    } catch (e) {
...
ipcMain.handle('calendar-delete-event', async (event, { eventId }) => {
    try {
        return await backendClient.post('/api/calendar', { action: 'delete_event', event_id: eventId });
    } catch (e) {
...
async function executeStep(step) {
    if (step.tool === 'execute_system_command') {
        ...
        await backendClient.post('/api/calendar', step.args).catch(e => console.warn('[Macro] calendar_action failed:', e.message));
    }
}
...
async function analyzeScreen(question = 'What is currently on the screen?') {
    try {
        ...
        return await backendClient.analyzeScreenImage(base64Image, fullQuestion);
    } catch (e) {
